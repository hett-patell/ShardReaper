...
When SHARDREAPER_LLM_BASE / SHARDREAPER_LLM_KEY / SHARDREAPER_LLM_MODEL are set, the
...
BASE = os.environ.get("SHARDREAPER_LLM_BASE", "https://api.openai.com/v1")
KEY = os.environ.get("SHARDREAPER_LLM_KEY", "")
MODEL = os.environ.get("SHARDREAPER_LLM_MODEL", "gpt-4o-mini")
TIMEOUT = int(os.environ.get("SHARDREAPER_LLM_TIMEOUT", "120"))
...
def cli_ask_question(task, scope, objective, max_tokens):
    if not available():
        print("no brain configured: set SHARDREAPER_LLM_BASE/SHARDREAPER_LLM_KEY/"
              "SHARDREAPER_LLM_MODEL (OpenAI-compatible). Deterministic mode active.")
        return
...
def build_arg_parser(sub):
    p = sub.add_parser("ask", help="ask the LLM brain a question (needs SHARDREAPER_LLM_*)")
