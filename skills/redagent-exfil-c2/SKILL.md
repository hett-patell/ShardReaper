...
# RedAgent Exfiltration & C2 — Take the Data, Keep the Channel
...
1. Locate the crown jewels: document shares, database backups, source repos, mail stores, cloud storage buckets. `redagent kb "collection document share database backup"` for playbooks.
...
3. Choose the channel that fits the network:
   - Web/HTTPS: `redagent kb "exfiltration http"` — T1041, T1567 (web service), T1048 (alternative protocol).
...
5. Stand up the framework: `redagent weapons c2` — Sliver (recommended: modern, cross-platform), Mythic, Empire, or dnscat2 for pure-DNS. 
6. Redundancy: two channels minimum — primary HTTPS, fallback DNS. Heartbeat-based reconnection. Implants that survive reboots (see redagent-persist).
