...
# RedAgent Credential Harvest — Take Every Secret
...
1. **Corpus first**: `redagent kb "credential dumping kerberos mimikatz"` — ired.team's `credential-access-and-credential-dumping/` is a full curriculum.
2. **Rack selection**: `redagent atomic select --technique T1003` (OS Credential Dumping — 8 sub-techniques), plus `kerberoast` keyword search.
3. **Weapons**: `redagent weapons credential-access` — mimikatz, Rubeus, LaZagne, responder, hashcat, john...
...
5. **Crack offline**: hashcat/john on harvested hashes (`redagent weapons credential-access`).
