...
# RedAgent Lateral Movement — Spread Through the Estate
...
1. **Corpus first**: `redagent kb "lateral movement psexec winrm pass the hash"`.
2. **Rack selection**: `redagent atomic select "lateral movement"` — plus T1021 family (Remote Services: T1021.001 RDP, .002 SMB, .003 WinRM, .004 SSH, .005 VNC).
3. **Weapons**: `redagent weapons lateral-movement` — impacket (psexec/wmiexec/smbexec), evil-winrm, NetExec/CrackMapExec, chisel/ligolo for pivots.
