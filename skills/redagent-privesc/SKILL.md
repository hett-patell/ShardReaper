...
# RedAgent Privilege Escalation — Take Everything
...
1. **Corpus first**: `redagent kb "privilege escalation enumeration"` — the corpus returns the full checklists and exploit writeups.
2. **Enumeration weapons**: `redagent weapons privilege-escalation` — LinPEAS / WinPEAS (PEASS-ng), PowerUp, Seatbelt. Run them; read everything they print.
3. **Rack selection**: `redagent atomic select "privilege escalation" --platform <linux|windows>`.
