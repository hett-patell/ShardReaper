...
# RedAgent Defense Evasion — Operate Under Their Noses
...
1. **Corpus first**: `redagent kb "defense evasion amsi bypass obfuscation"` — ired.team's `defense-evasion/` has the deep writeups.
2. **Rack selection**: `redagent atomic select "defense evasion"` — T1562 (impair defenses), T1070 (indicator removal), T1027 (obfuscated files), T1218 (signed binary proxy), T1055 (process injection).
3. **Weapons**: `redagent weapons defense-evasion` — veil, donut, macro_pack, and the process-injection writeups in the corpus.
...
5. **Before every action ask**: what artifact does this create, and how do I remove it? Cleanup commands for every atomic test are available (`redagent atomic run --cleanup`).
