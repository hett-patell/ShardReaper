...
requires an explicit --go. This turns the corpus into RedAgent's weapons rack.
...
def _mini_yaml(text):
    ...
    Extracts the fields RedAgent actually uses: technique id, display_name,
...
def cli_select(args):
    ...
    print("\nrun: redagent atomic run <idx> --go")
...
def cli_run(args, idx=None):
    ...
    if not tests:
        print("no test selected — use --technique T#### or --index N (see: redagent atomic select)")
        return 1
    test = tests[0]
