...
RedAgent is a knowledge-driven agent: it routes every phase to the right
...
def corpus_roots():
    """Locate every reference corpus beside the project (or under REDAGENT_CORPUS)."""
    base = os.environ.get("REDAGENT_CORPUS")
    if base:
...
def cli_search(query, limit=8, corpus=None):
    ...
    for r in results:
        ...
        if r["desc"]:
            print(f"      {r['desc']}")
    print()
    print("open with: redagent kb-open <query>   |   full path above")


if __name__ == "__main__":
    import argparse
    ap = argparse.ArgumentParser(description="RedAgent knowledge base")
